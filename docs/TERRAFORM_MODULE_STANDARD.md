...
- Do not pass application secret values through Terraform when a service-native
  generated credential or runtime secret reference is available.
- Container modules accept image and secret references, not registry
  credentials or plaintext application secrets.
...
The source template may publish a clean, validation-gated snapshot to a staging
GitHub repository. That publication is not an AWS deployment. It must:

- run only after formatting, lint, security, module-test, and example jobs pass
- run only from the canonical source repository's `main` branch
- use a deploy key scoped only to the staging repository
- recreate the staging branch from the validated source snapshot
- exclude Git internals, Terraform working directories, and lock files
- contain a source commit reference in the staging snapshot commit message

Developers must not configure a local Git remote for the staging repository or
push staging snapshots from their workstation.
