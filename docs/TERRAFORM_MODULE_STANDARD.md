...
- run `terraform plan` against an AWS account (credential-free offline plans
  using placeholder credentials, disabled provider validation, no refresh,
  and no backend are permitted)
...
AWS deployment is not part of this template workflow. A separately configured
GitHub stage-repository snapshot may be published after successful validation;
that job must use an explicitly configured repository and token, skip pull
requests, prevent source-repository loops, and never publish to AWS.
